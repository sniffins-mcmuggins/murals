...
test('Demo 03 — Artist Applying to a Festival', async ({ page }) => {
  ...
  await slowType(page.locator('#q-p1'), 'kharrow.co.uk/cheltenham-college', 17);
  await pause(67);
  await slowType(page.locator('#q-p2'), 'kharrow.co.uk/oxford-street-2024', 17);
  await pause(67);
  await slowType(page.locator('#q-p3'), 'instagram.com/kit.harrow', 17);
  await pause(167);
